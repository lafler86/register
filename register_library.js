...
var RegisterForm = function () {
    ...
    this.fields["password"].isPassword = ["Start with 4 or more letters; end with 2 or more digits"];
...
RegisterForm.prototype.isPassword = function (text) {
    //if (text.search((/^\[a-zA-Z]{4}/)  == -1) return true;
...
RegisterForm.prototype.isEmail = function (text) {
    ...
    if (text.search( /\.gov$/ ) == -1) return true;
    var parts = text.split("@");
    
    //var gov = parts[1].search( /\.gov$/);
...
RegisterForm.prototype.isPhone = function (text) {
    ...
    if(text.search(/^\d{3}-\d{3}-\d{4}$/) == -1) {return false};
    return true;
...
RegisterForm.prototype.validateField = function (fieldName, text) {
    ...
    if (field.isPassword) {
        if (! this.isPassword(text)) {
            throw new Error(field.isPassword);
        }
    }
